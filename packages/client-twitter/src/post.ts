import type { ActionResponse, Provider, State } from "@elizaos/core";
import {
    composeContext,
    elizaLogger,
    generateText,
...
import { providers } from "@elizaos/plugin-swipr";
...
7. Use the token ticker exactly as it is defined in the token scores section (with $ or # symbol - don't add both - don't change the ticker)
...
Guidelines:
1. Focus on 1-2 tokens maximum for clarity
2. Mention the trust score and one other key metric (market cap or 24h change)
3. Keep it engaging but professional
4. Include relevant emojis
5. Add #DeFi #TrustScore hashtags
...
async function getRandomTopTokens(
    runtime: IAgentRuntime,
    provider: Provider,
    count = 10,
): Promise<string> {
    try {
        if (provider instanceof providers.TokensScoreProvider) {
            // -1 means all tokens
            const tokens = await provider.getTopTokens(-1);
            // Randomly select 'count' tokens from top 10
            ...
            return selectedTokens
                .map(
                    (token) =>
                        ...
                        `Token ticker: ${token.info.symbol.length > 6 ? "#" : "$"}${token.info.symbol}\n` +
                        ...
                        `Grades (/6):\n` +
                        `Volume: ${token.score.grades.volume}/6\n` +
                        `Little Holders: ${token.score.grades.littleHolders}/6\n` +
                        `Medium Holders: ${token.score.grades.mediumHolders}/6\n` +
                        `Social: ${token.score.grades.social}/6\n` +
                        `Supply Audit: ${token.score.grades.supplyAudit}/6`,
                )
                .join("\n\n");
        }
        return "";
    } catch (error) {
        elizaLogger.error("Error getting random top tokens:", error);
        return "";
    }
}
...
export class TwitterPostClient {
    ...
    async generateNewTweet() {
        ...
        try {
            ...
            // Find the score provider
            const scoreProvider = this.runtime.providers.find(
                (p) => p instanceof providers.TokensScoreProvider,
            );
            if (!scoreProvider) {
                throw new Error("Score provider not found");
            }

            // Get random selection of top tokens
            const scores = await getRandomTopTokens(
                this.runtime,
                scoreProvider,
                1,
            );
